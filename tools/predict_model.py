...
($TPV_DATA/ro_predict.csv) and the SDF boundary sweep ($TPV_DATA/sdfsim.csv) --
into the frozen prediction package written to $TPV_DATA/predict/. TPV_DATA
defaults to data/halfcycle; tools/verify_safe10.sh sets it to data/safe10 for
the final 10 MHz build. The archived root-level data/predict/ package is
historical v1 evidence and is not regenerated by this model version.
...
Run: python3 tools/predict_model.py [--outdir DIR]  # default $TPV_DATA/predict
        ...
        for corner in C.CORNER_NAMES:
            for can in CANARIES:
                ...
                if c > COUNTER_MAX or sat:
                    ok = False
                counts["%s@%s" % (can, corner)] = {"count": c, "would_wrap": sat}
...
def write_summary(path, prov, n_in, cross, ro_idx, ratios, lut, sta_idx,
    ...
    L.append("")
    data_dir = os.path.relpath(C.DATA, C.REPO)
    L.append("- Inputs: `%s/experiment_sta.csv` (%d rows), "
             "`%s/ro_predict.csv` (%d rows), `%s/sdfsim.csv` (%d rows)"
             % (data_dir, n_in["sta"], data_dir, n_in["ro"], data_dir,
                n_in["sdf"]))
    ...
    L.append("Each predictor x is calibrated at the primary nominal anchor "
             "(1.20 V, 25 C, seg3333, worst). The v2 protocol has no fallback "
             "anchor: if the primary boundary is censored above the 50 MHz "
             "ceiling, calibration is reported unavailable "
             "(`docs/prediction-model.md`). The rule is k_x = F_meas / "
             "P_x(anchor), applied as P_x_cal = k_x * P_x everywhere, "
    ...
    json_doc = {
        ...
        "calibration": {
            ...
            "note": "post-silicon run substitutes measured k_x per predictor "
                    "at the primary anchor and re-emits; nothing else changes. "
                    "v2 has no fallback anchor: if the primary boundary is "
                    "censored above the 50 MHz ceiling, calibration is "
                    "unavailable (docs/prediction-model.md)",
        },
        "notes": [
