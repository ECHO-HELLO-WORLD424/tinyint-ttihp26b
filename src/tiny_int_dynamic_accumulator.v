...
// Bit-exact 20-bit accumulator split into five independently enabled nibbles.
// accumulator_mode uses the transaction encoding from the external protocol:
//   2'b01: 8-bit active region,  2'b10: 12-bit active region,
//   2'b11: 16-bit active region.
//
// The addend contract is the output of product_extender: an unsigned 8-bit
// value zero-extended to 20 bits, or a signed 8-bit value sign-extended to
// 20 bits. Cold stages therefore see only an increment, decrement, or hold
// event. No combinational or ripple-generated clock is used.
module tiny_int_dynamic_accumulator (
    input  wire        clk,
    input  wire        rst_n,
    input  wire        clear,
    input  wire        load,
