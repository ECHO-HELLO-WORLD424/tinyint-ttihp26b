...
// P4 adaptive-boundary two-compilation equivalence driver.
//
// The same testbench is compiled twice and the cycle logs are diffed:
//   1. Golden run:   iverilog -g2012 -DREF_CORE p4_core_equiv_tb.v
//                    test/p4_ref/tiny_int_core_ref.v (the pre-proposal
...
//   2. Proposal run: iverilog -g2012 p4_core_equiv_tb.v
...
//
// Documented P4 deviation: the proposed core retunes the effective boundary
// while the golden core runs the latched mode, and READ selector 3'b110
// reports {2'b00, effective_boundary, ...} instead of {4'b0000, ...}. Every
// boundary policy is bit-exact (identical stored value and identical true
// carry), so the only log difference is the held response of that selector:
// its upper nibble is masked to 'Z' in both runs, keeping the configuration
// low nibble (zero_skip | mode | signed) compared.
module p4_core_equiv_tb;
...
`endif
  ...
  // True when the held response_data byte came from the configuration
  // selector 3'b110, whose upper nibble is the one documented P4 deviation.
  reg held_config_response;
  ...
  // Shared per-cycle log line. The held configuration response (selector
  // 3'b110) is the one documented P4 deviation, so its upper nibble is
  // masked to 'Z' to keep the two compilation logs byte-identical.
  task log_cycle;
    begin
      if (held_config_response) begin
        $fdisplay(fd, "t=%0t val=%05x ovf=%b rsp=Z%01x rspv=%b cnt=%02x done=%b last=%02x covf=%b cof=%b perr=%b mult=%02x ext=%05x rdy=%b lsm=%b ares=%05x acry=%b aovf=%b",
                  $time, accumulator_value, accumulator_overflow,
                  response_data[3:0], response_valid, pair_count, done,
  ...
  task step;
    ...
    begin
      ...
      if (valid && command == 3'b100) begin
        held_config_response = (data[2:0] == 3'b110);
      end
    end
  endtask
  ...
  initial begin
    ...
    $fclose(fd);
    $display("PASS p4_core_equiv mode=%0d workload=%0d", mode, workload);
    $finish;
  end
